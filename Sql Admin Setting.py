...
def process_column_and_generate_query(input_string, output_file, log_file, table_name, account_id):
    try:
        # Get the database credentials from environment variables

        server = os.getenv('sqlDB_server')
        database = os.getenv('sqlDB_DATABASE')
        username = os.getenv('sqlDB_USER')
        password = os.getenv('sqlDB_PASSWORD')
        schema=os.getenv('sqlschema')
        ...
        for entry in entries:
            ...
            if column_type in ['dropdown', 'multiselect']:
                ...
                query = f"SELECT DISTINCT [{column_name_full}] FROM [{database}].[{schema}].[{table_name}]"
...
def process_column_and_update_query(input_string, Update_file, log_file, mappable_file, table_name, account_id):
    try:
        ...
        for entry in entries:
            ...
            query = f"select 1 from INFORMATION_SCHEMA.columns where column_name='{column_name_full}' and table_name='{table_name}' and TABLE_SCHEMA ='{schema}' AND TABLE_CATALOG='{database}' ;"
            cursor.execute(query)
        ...
        with open(log_file, "a", encoding="utf-8") as file:
            file.write(f"Unexpected Error: {ex}\n")
...
if table_name.lower()=='candidate_custom_data':
    entity='Candidate'
elif table_name.lower()=='contact_custom_data':
    entity='Contact'
elif table_name.lower()=='job_custom_data':
    entity='Job'
elif table_name.lower()=='company_custom_data':
    entity='Company'
elif table_name.lower()=='deal_custom_data':
    entity='Deal'
