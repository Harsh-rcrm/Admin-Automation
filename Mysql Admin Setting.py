    ...
    try:
        ...
        for entry in entries:
            ...
            if column_type in ['dropdown', 'multiselect']:
                ...
                query = f"SELECT DISTINCT `{column_name_full}` FROM `{table_name}`"
...
def process_column_and_update_query(input_string, Update_file, log_file, mappable_file, table_name, account_id ):
    try:
        ...
        for entry in entries:
            ...
            query = f"SELECT 1 FROM information_schema.columns WHERE table_name='{table_name}' AND table_schema='{database}' AND column_name='{column_name_full}';"
            cursor.execute(query)
            ...
            if column_type in ['dropdown', 'multiselect']:
                ...
                cursor.execute(f"SELECT DISTINCT `{column_name_full}` FROM `{table_name}`")
                results = cursor.fetchall()
                distinct_values = [row[0] for row in results if row[0] is not None]
